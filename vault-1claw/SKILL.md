...
Pick whichever rail you can pay on. Sign an EIP-3009
`TransferWithAuthorization` for the chosen asset on its chain, base64-encode
the signed payload, and send it as the `X-PAYMENT` header when you replay the
original request. The `extra` object is the EIP-712 domain the signature must
use.
